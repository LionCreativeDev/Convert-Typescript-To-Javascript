...
const transformTsToJs = (node) => {
  ...
  node.children.forEach((childNode) => {
    if (childNode.children && childNode.children.length) {
	  ...
	  if (childNode.path.includes('.css') || childNode.path.includes('.scss') || childNode.path.includes('.ttf') || childNode.path.includes('.otf') || childNode.path.includes('.woff') || childNode.path.includes('.woff2') || childNode.path.includes('.eot')) {
        return;
      }
	  
	  if (childNode.path.includes('.png') || childNode.path.includes('.jpg') || childNode.path.includes('.gif') || childNode.path.includes('.jpeg') || childNode.path.includes('.svg') || childNode.path.includes('.txt') || childNode.path.includes('.webp')) {
        return;
      }
